...
[GitHub](https://github.com/expo/expo/tree/sdk-56/packages/expo-audio)[npm](https://www.npmjs.com/package/expo-audio)[Changelog](https://github.com/expo/expo/tree/sdk-56/packages/expo-audio/CHANGELOG.md)
...
Adds `FOREGROUND_SERVICE` and `FOREGROUND_SERVICE_MEDIA_PLAYBACK` permissions. Also declares a media playback foreground service (`AudioControlsService`) in app's AndroidManifest.xml.
  * iOS

Adds the `audio` `UIBackgroundMode` capability
...
Adds `FOREGROUND_SERVICE`, `FOREGROUND_SERVICE_MICROPHONE`, and `POST_NOTIFICATIONS` permissions. Also declares an audio recording foreground service in app's AndroidManifest.xml.
  * iOS
...
---|---|---
source(optional)| `AudioSource`| The audio source to load. Can be a local asset via `require()`, a remote URL, or null for no initial source.Default:`null`
options(optional)| `AudioPlayerOptions`| Audio player configuration options.Default:`{}`
...
Parameter| Type| Description
---|---|---
options(optional)| `AudioPlaylistOptions`| Audio playlist configuration options including initial sources and loop mode.Default:`{}`
...
Parameter| Type| Description
---|---|---
playlist| `AudioPlaylist`| The `AudioPlaylist` instance to monitor.
...
Parameter| Type
---|---
options(optional)| `AudioStreamOptions`
...
Type: Class extends `[SharedObject](/versions/v56.0.0/sdk/expo#sharedobjecttype)<AudioEvents>`
...
Parameter| Type| Description
---|---|---
rate| `number`| The playback rate of the audio. See `playbackRate` property for detailed range information.
...
Type: Class extends `[SharedObject](/versions/v56.0.0/sdk/expo#sharedobjecttype)<AudioPlaylistEvents>`
...
Parameter| Type| Description
---|---|---
source| `AudioSource`| The audio source to add.
...
Parameter| Type| Description
---|---|---
source| `AudioSource`| The audio source to insert.
...
Type: Class extends `[SharedObject](/versions/v56.0.0/sdk/expo#sharedobjecttype)<RecordingEvents>`
...
`[Promise](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/Promise)<RecordingInput>`
...
Parameter| Type
---|---
options(optional)| `[Partial](https://www.typescriptlang.org/docs/handbook/utility-types.html#partialtype)<RecordingOptions>`
...
Type: Class extends `[SharedObject](/versions/v56.0.0/sdk/expo#sharedobjecttype)<AudioStreamEvents>`
...
Parameter| Type| Description
---|---|---
source| `AudioSource`| The audio source to release. Must match the source previously passed to `preload()`.
...
Parameter| Type| Description
---|---|---
options(optional)| `AudioPlaylistOptions`| Audio playlist configuration options.Default:`{}`
...
Parameter| Type| Description
---|---|---
source| `AudioSource`| The audio source to preload. Can be a URL string, a local asset via `require()`, or an audio source object.
options(optional)| `PreloadOptions`| Optional configuration for preloading behavior.Default:`{}`
...
Parameter| Type| Description
---|---|---
mode| `[Partial](https://www.typescriptlang.org/docs/handbook/utility-types.html#partialtype)<AudioMode>`| Partial audio mode configuration object. Only specified properties will be updated.
...
Property| Type| Description
---|---|---
allowsBackgroundRecording(optional)| `boolean`| Only for: AndroidiOS
Whether audio recording should continue when the app moves to the background.Default:`false`
allowsRecording(optional)| `boolean`| Only for: iOS
Whether the audio session allows recording.Default:`false`
interruptionMode(optional)| `InterruptionMode`| Determines how the audio session interacts with other audio sessions.
...
AndroidDetermines how the audio session interacts with other sessions on Android.
...
If set to `true`, the system will attempt to download the resource to the device before loading. This value defaults to `false`. Works with:
...
keepAudioSessionActive(optional)| `boolean`| Only for: iOS
If set to `true`, the audio session will not be deactivated when this player pauses or finishes playback. This prevents interrupting other audio sources (like videos) when the audio ends. Useful for sound effects that should not interfere with ongoing video playback or other audio. The audio session for this player will not be deactivated automatically when the player finishes playback.
...
How often (in milliseconds) to emit playback status updates. Defaults to 500ms.Default:`500ms`Example
...
Property| Type| Description
---|---|---
playlistStatusUpdate| `(status: AudioPlaylistStatus) => void`| Fired when the playlist's status changes (play/pause/seek/load/track change).
...
---|---|---
crossOrigin(optional)| `'anonymous' | 'use-credentials'`| Only for: Web
Sets the `crossOrigin` attribute on the `<audio>` elements used for playback. Required for CORS-enabled audio files when you need to access audio data.Default:`undefined`
loop(optional)| `AudioPlaylistLoopMode`| Loop mode for the playlist.
...
Default:`'none'`
sources(optional)| `AudioSource[]`| Initial sources to add to the playlist. Each source can be a local asset, remote URL, or null.Default:`[]`
...
loop| `AudioPlaylistLoopMode`| Current loop mode.
...
encoding(optional)| `AudioStreamEncoding`| PCM encoding format.Default:`'float32'`
onBuffer(optional)| `(buffer: AudioStreamBuffer) => void`| Called each time a new PCM buffer is captured from the microphone.
...
web| `RecordingOptionsWeb`| Only for: Web
...
Property| Type| Description
---|---|---
audioEncoder| `AndroidAudioEncoder`| The desired audio encoder. See the `AndroidAudioEncoder` type for all valid values.
audioSource(optional)| `RecordingSource`| The desired audio Source. See the `RecordingSource` type for all valid values.
...
outputFormat| `AndroidOutputFormat`| The desired file format. See the `AndroidOutputFormat` type for all valid values.
...
The time in seconds to wait before starting the recording. If not provided, recording starts immediately. Platform behavior:
